...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for p in self.legalPositions:
            #print p
            #print self.getPositionDistribution(self.setGhostPosition(gameState, p))
            newPos = self.getPositionDistribution(self.setGhostPosition(gameState, p))
            for p2,pr2 in newPos.items():
                newDist[p2] += pr2*self.beliefs[p]

        #for p3 in newDist:
        #    newDist[p3] = newDist[p3]*self.beilefs[p3]
        newDist.normalize()
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        particlesPerPosition = int(numParticles/len(legalPositions))
        particles = []
        # store particles as list
        ...
        # still some left over particles which we can assign in order
        i = 0
        while i < (numParticles - particlesPerPosition*len(legalPositions)):
            particles += [legalPositions[i]]
            i += 1
    ...
    def observe(self, observation, gameState):
        ...
        # if ghost was eaten, adjust all particles to the jail
        if noisyDistance == None:
            newParticles = list()
            while len(newParticles) < self.numParticles:
                newParticles += [self.getJailPosition()]
            self.particles = newParticles
            return

        # weights will contain an item for each possible position
        # and a probability for being in that state
        weights = util.Counter()
        # first step is to calculate weights of new particles
        # observed a distance to a ghost, this effects the particles
        for particle in self.particles:
            #print particle
            # first calculate distance to use the emissionModel
            particleDistance = util.manhattanDistance(pacmanPosition, particle)
            #print emissionModel[particleDistance]
            weights[particle] += emissionModel[particleDistance]
        ...
        newParticles = list()
        # generate random number, go through totalWei, checking if number is big enough
        # when number is big enough, assign particle to part
        #for i in range(1,self.numParticles):
        while len(newParticles) < self.numParticles:
            choiceVar = random.random()
            cummWeight = 0
            for loc,wei in weights.items():
                cummWeight += wei
                if choiceVar < cummWeight:
                    # this is the right particle to assign to
                    # no need to repeat after this though
                    newParticles += [loc]
                    cummWeight = -1
    ...
    def elapseTime(self, gameState):
        ...
        # define a new belief system
        newParticles = list()
        newBeliefs = util.Counter()
        # take each particle and evolve it one time step using prob distro
        # calculate new beliefs
        for particle in self.particles:
            #print self.getPositionDistribution(self.setGhostPosition(gameState, p))
            newPos = self.getPositionDistribution(self.setGhostPosition(gameState, particle))
            # use this position distribution to assign new location
            newParticles += [util.sample(newPos)]

        # new particles are defined, calculate new beliefs
        self.particles = newParticles
    ...
    def getBeliefDistribution(self):
        ...
        stateCounts = util.Counter()
        
        #print "getBeliefDistribution"
        #print self.particles        
        for particle in self.particles:
            stateCounts[particle] += 1

        stateCounts.normalize()

        self.beliefs = stateCounts
        #print self.beliefs
        return self.beliefs
        #util.raiseNotDefined()
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        particlesPerPosition = int(numParticles/(numLegalPositions**numGhosts))
        particles = []

        # particles now consist of a list of multiple positions
        # depending on the number of ghosts, e.g. [ [1, 1], [1, 2], ...
        # repeat the operation for the same position for as many identical parts
        a = 1
        while a <= particlesPerPosition:
            i = 1     
            while i <= numLegalPositions**numGhosts:
                j = 1
                while j <= numGhosts:
                    if j == 1:
                        part = (legalPositions[i % numLegalPositions+1-1])  
                    else:
                        part = (part,) + (legalPositions[int(i / (numLegalPositions**(j-1)) )
                                                      % numLegalPositions + 1-1],)
                    j += 1
        ...
        # still some left over particles which we should assign randomly
        # use random then sample from the first pos**ghos to pick randomss
        # this will probably double up though...
        # this breaks down if there are positions then there are particles
        #print "ppp",particlesPerPosition
        #print "gh",numGhosts
        #print "nP",numParticles
        #print "p",particles
        #print "nLP",numLegalPositions
        while len(particles) < numParticles:
            randParticle = ()
            i = 0
            while i < numGhosts:
                randParticle += (legalPositions[random.randint(1,numLegalPositions)-1],)
                i += 1
            #print randParticle
    ...
    def observeState(self, gameState):
        ...
        # if ghost was eaten, adjust all particles to the jail
        noisyCheck = 0
        #print "emissionModel",emissionModels
        for index, noisyDistance in enumerate(noisyDistances):
            # for all ghosts in jail, assign that position,
            # if all assigned to jail, can insta-return
            # if not all in jail, must continue
            if noisyDistance == None:
                #print noisyDistances
                newParticles = ()
                # one of the ghosts is in jail, update all
                # particles to put him in jail
                for particle in self.particles:
                    newParticles += (self.getParticleWithGhostInJail(particle,index),)
        
                self.particles = newParticles
                noisyCheck += 1

        if noisyCheck == len(noisyDistances):
            #print "good"
            return
        ...
        for particle in self.particles:
            ...
            particleDistances = ()
            #print particle
            for index, ghost in enumerate(particle):
                particleDistances += (util.manhattanDistance(pacmanPosition, particle[index]),)

            probPart = 1
            
            for index, ghost in enumerate(particleDistances):
                if noisyDistances[index] == None:
                    multPart = 1
                else:
                    multPart = emissionModels[index][particleDistances[index]]
                probPart = probPart * multPart
 
            weights[particle] += probPart
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            # calculate new beliefs
            thisParticle = ()
            for index, ghost in enumerate(newParticle):
                #print index, ghost
                newPosDist = getPositionDistributionForGhost(
                    setGhostPositions(gameState, oldParticle), index,
                    self.ghostAgents[index])
                # use this position distribution to assign new location
                thisParticle += (util.sample(newPosDist),)
            #print thisParticle
            newParticle = thisParticle
            # new particles are defined, calculate new beliefs
